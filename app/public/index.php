...
if (preg_match('#^itens/(\d+)/excluir$#', $path, $m)) {
    (new \Controllers\ItemController())->destroy((int)$m[1]); exit;
}
...
if ($path === 'colaboradores/autocomplete') {
    (new \Controllers\ColaboradorController())->autocompleteColaboradores(); exit;
}

if ($path === 'colaboradores/calendario/enviar-email') {
    (new \Controllers\ColaboradorController())->enviarEmailAniversario(); exit;
}

if ($path === 'colaboradores/calendario/aniversariantes') {
    (new \Controllers\ColaboradorController())->calendarioAniversariantesJson(); exit;
}

if (preg_match('#^colaboradores/(?!autocomplete$|organograma$|calendario(?:/|$))(.+)$#', $path, $m)) {
    (new \Controllers\ColaboradorController())->showColaborador(urldecode($m[1])); exit;
}
...
switch ($path) {
	...
	case 'colaboradores':
		$c = new \Controllers\ColaboradorController();
		$c->colaboradores();
		break;

	case 'colaboradores/autocomplete':
		$c = new \Controllers\ColaboradorController();
		$c->autocompleteColaboradores();
		break;

	case 'colaboradores/organograma':
		$c = new \Controllers\ColaboradorController();
		$c->organograma();
		break;

	case 'colaboradores/calendario':
		$c = new \Controllers\ColaboradorController();
		$c->calendarioAniversarios();
		break;

	// RH — Composição Remuneração
	// case 'rh/composicao-remuneracao':
	// 	$c = new \Controllers\RhController();
	// 	$c->index();
	// 	break;
