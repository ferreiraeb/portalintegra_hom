...
private function pageFetch(string $op, string $tipo, array $filters, int $pp, int $p): array
{
    ...
    private function exportFromDb(): void
    {
        ...
        switch ($tab) {
            case 'sys':
                $tipo     = 'ONLY_SYS';
                $columns  = 'Empresa, ProdutoCodigo, ProdutoReferencia, ProdutoDescricao, NCMCodigo, NCMIdentificador';
                $filename = 'massey_dealernet.csv';
                break;
            case 'plan':
                $tipo     = 'ONLY_PLAN';
                $columns  = 'ProdutoReferencia, ProdutoDescricao, ValorPlanilha';
                $filename = 'massey_planilha.csv';
                break;
            default: // 'match'
                $tipo     = 'MATCH';
                $columns  = 'Empresa, ProdutoCodigo, ProdutoReferencia, ProdutoDescricao, NCMCodigo, NCMIdentificador, ValorPlanilha, Overprice, IndiceParaCusto, PrecoPublico, PrecoSugerido, PrecoGarantia, PrecoReposicao';
                $filename = 'massey_result.csv';
                break;
        }

        // Token opcional enviado pelo JS para detectar (via cookie) quando o arquivo começou a ser enviado
        ...
        $st = $pdo->prepare("
            SELECT {$columns}
            FROM [Portal_Integra].dbo.MF_Result
            WHERE OpID = CAST(? AS uniqueidentifier) AND Tipo = '{$tipo}'
            ORDER BY ProdutoDescricao, ProdutoReferencia
        ");
        $st->execute([$op]);
        $dados = $st->fetchAll(); // parte lenta — só depois disso iniciamos o output

        // Sinaliza ao JS via cookie (httponly=false para ser legível por document.cookie)
        if ($token !== '') {
            setcookie('mf_csv_ready', $token, [
                'expires'  => time() + 120,
                'path'     => '/',
                'samesite' => 'Lax',
                'httponly' => false,
