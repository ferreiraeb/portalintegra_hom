...
class AuthController {
    ...
    public function login() {
        if (is_post()) {
            ...
            // Permite login com e-mail: se não achou por login, tenta pela coluna email
            if (!$user && strpos($loginNorm, '@') !== false) {
                $user = $this->auth->findByEmail($loginNorm);
            }
            ...
            if ($this->cfg['ldap']['enabled']) {
                // Determina a credencial correta para o bind LDAP.
                // Quando o usuário digita um e-mail, o endereço mail do AD pode ser diferente
                // do UPN (login@valence.ad), então precisamos resolver o UPN real.
                $ldapCredential = $login;   // padrão: o que foi digitado
                $ldapLoginKey   = $loginKey;

                if (strpos($loginNorm, '@') !== false) {
                    if ($user) {
                        // Usuário encontrado pelo campo email — usa UPN ou sAMAccountName+sufixo
                        $suffix = $this->cfg['ldap']['user_upn_suffix'] ?? '';
                        $ldapCredential = $user['upn'] ?? ($user['login'] . $suffix);
                        $ldapLoginKey   = $user['login'];
                    } else {
                        // Usuário não existe no DB ainda — busca sAMAccountName pelo mail no LDAP
                        $ldapUser = $this->ldap->findUserByMail($loginNorm);
                        if ($ldapUser && !empty($ldapUser['sAMAccountName'])) {
                            $suffix = $this->cfg['ldap']['user_upn_suffix'] ?? '';
                            $ldapCredential = $ldapUser['upn'] ?? ($ldapUser['sAMAccountName'] . $suffix);
                            $ldapLoginKey   = strtolower($ldapUser['sAMAccountName']);
                            // Se já existir no DB com o login real, aproveita o registro
                            $user = $this->auth->findByLogin($ldapLoginKey) ?: $user;
                        }
                    }
                }

                $ok = $this->ldap->authenticateUserPassword($ldapCredential, $password);
                if ($ok) {
                    if (!$user) {
                        // Cria com mínimo de campos; permissões atribuídas depois pelo admin
                        ...
                        $user = $this->auth->findByLogin($ldapLoginKey);
                    } else {
                        // Garante origem 'ad'; recarrega pelo login armazenado (evita bug de case)
                        if ($user['origem'] !== 'ad') {
