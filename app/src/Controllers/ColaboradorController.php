...
class ColaboradorController
{
    ...
    public function calendarioAniversarios(): void
    {
        ...
        try {
            ...
            $aniversariantesFim  = ((int)$hoje->format('N') === 1)
                ? $service->getAniversariantesDoFimDeSemana($hoje)
                : [];
        ...
        render_page('colaboradores/calendario.php', [
            ...
            'aniversariantesFim'  => $aniversariantesFim,
            'isSegunda'           => (int)$hoje->format('N') === 1,
            'isFimDeSemana'       => (int)$hoje->format('N') >= 6,
    ...
    public function calendarioAniversariantesJson(): void
    {
        ...
        try {
            ...
            $isMonday = $weekday === 1;
            $fim      = $isMonday ? $service->getAniversariantesDoFimDeSemana($parsed) : [];
            ...
            echo json_encode([
                ...
                'isSegunda'        => $isMonday,
                'isFimDeSemana'    => $weekday >= 6,
                'fimDeSemanaCount' => count($fim),
            ]);
        } catch (\Throwable $e) {
    ...
    public function enviarEmailAniversario(): void
    {
        ...
        $refDate = $this->parseCalendarioDate((string)($_POST['date'] ?? ''));
        if ($refDate === null) {
            echo json_encode(['ok' => false, 'error' => 'Selecione um dia válido no calendário.']);
            return;
        }

        try {
            $aniversarioService = new \Services\AniversarioService();
            $weekday            = (int)$refDate->format('N');
            $isWeekend          = $weekday >= 6;
            $isMonday           = $weekday === 1;
            ...
            if (empty($aniversariantes) && empty($weekendPeople)) {
                echo json_encode(['ok' => false, 'error' => 'Nenhum aniversariante para enviar nesta data.']);
                return;
            }
            ...
            if (!empty($weekendPeople)) {
                $birthdayEmailService->sendWeekendGroupEmail($weekendPeople, false, $recipients, $refDate);
                $parts[] = sprintf(
                    'resumo do fim de semana (%d)',
                    count($weekendPeople)
                );
            }

            if ($isWeekend) {
                $parts[] = 'grupo do dia omitido (fim de semana)';
            } elseif (!empty($aniversariantes)) {
                $birthdayEmailService->sendGroupEmail($aniversariantes, false, $recipients, $refDate);
                $parts[] = 'resumo de grupo do dia';
            }
            ...
            if (!empty($aniversariantes)) {
                ...
                $parts[] = sprintf('%d e-mail(s) individual(is)', count($sentIndividuals));
            }

            $nomesDia  = array_map(fn(array $r) => (string)($r['NOMECOMPLETO'] ?? ''), $aniversariantes);
            $nomesFim  = array_map(fn(array $r) => (string)($r['NOMECOMPLETO'] ?? ''), $weekendPeople);
            $dataLabel = $refDate->format('d/m/Y');

            echo json_encode([
                'ok'      => true,
                'message' => sprintf(
                    'E-mails enviados para %s (%s): %s.%s%s',
                    implode(', ', $recipients),
                    $dataLabel,
                    implode('; ', $parts),
                    $nomesDia ? ' Nesta data: ' . implode(', ', $nomesDia) . '.' : '',
                    $nomesFim ? ' Fim de semana: ' . implode(', ', $nomesFim) . '.' : ''
                ),
                ...
                'weekend_count'        => count($weekendPeople),
            ]);
        } catch (\Throwable $e) {
            http_response_code(500);
            echo json_encode(['ok' => false, 'error' => $e->getMessage()]);
    ...
    private function parseCalendarioDate(string $raw): ?\DateTimeImmutable
    {
        $raw = trim($raw);
        if ($raw === '') {
            return null;
    ...
    public function colaboradores(): void
    {
        ...
        try {
            ...
            $linhaCorporativaMap = $this->buildLinhaCorporativaMap($colaboradores);
            ...
            $colaboradores = array_map(function ($row) use ($usuarioEmailMap, $linhaCorporativaMap) {
                ...
                $row['linha_corporativa'] = $this->lookupLinhaCorporativa($row, $linhaCorporativaMap);
                return $row;
            }, $colaboradores);
    ...
    /**
     * Mapa de linhas corporativas por CPF e, como fallback, por nome+CODPESSOA.
     *
     * @param  array<int, array<string, mixed>> $colaboradores
     * @return array{by_cpf: array<string, string>, by_nome_cod: array<string, string>}
     */
    private function buildLinhaCorporativaMap(array $colaboradores): array
    {
        $map = ['by_cpf' => [], 'by_nome_cod' => []];
        ...
        foreach ($colaboradores as $row) {
            ...
            $cod = trim((string)($row['CODPESSOA'] ?? ''));
            if ($cod !== '') {
                $codpessoas[] = $cod;
            }
        }
        $cpfs = array_values(array_unique($cpfs));
        $codpessoas = array_values(array_unique($codpessoas));
        if (empty($cpfs) && empty($codpessoas)) {
            return $map;
        }
        ...
        try {
            ...
            $binds = [];
            if ($cpfs) {
                $ph = implode(',', array_map(fn($i) => ":cpf{$i}", array_keys($cpfs)));
                $conds[] = "e.colaborador_cpf IN ({$ph})";
                foreach ($cpfs as $i => $cpf) {
                    $binds[":cpf{$i}"] = $cpf;
                }
            }
            if ($codpessoas) {
                $ph = implode(',', array_map(fn($i) => ":cp{$i}", array_keys($codpessoas)));
                $conds[] = "e.colaborador_codpessoa IN ({$ph})";
            ...
            $st = $sqlSrv->prepare("
                SELECT e.colaborador_cpf,
                       e.colaborador_codpessoa,
                       e.colaborador_nome,
                       COALESCE(l_direct.numero_linha, l_linked.numero_linha) AS numero_linha
                " . $this->linhaCorporativaFromSql() . "
                WHERE  e.status IN ('ativo', 'reservado')
            ...
            foreach ($st->fetchAll(\PDO::FETCH_ASSOC) as $row) {
                ...
                $cpfKey = normalize_cpf($row['colaborador_cpf'] ?? '');
                if (strlen($cpfKey) === 11) {
                    $groupedCpf[$cpfKey][] = $num;
                }
                $nomeCod = $this->nomeCodKey(
                    (string)($row['colaborador_nome'] ?? ''),
                    (string)($row['colaborador_codpessoa'] ?? '')
                );
                if ($nomeCod !== "\0") {
                    $groupedNomeCod[$nomeCod][] = $num;
                }
            }

            foreach ($groupedCpf as $cpf => $numeros) {
                $map['by_cpf'][$cpf] = $this->formatLinhaCorporativaPlain($numeros);
            }
            foreach ($groupedNomeCod as $key => $numeros) {
                $map['by_nome_cod'][$key] = $this->formatLinhaCorporativaPlain($numeros);
            }
        } catch (\Throwable $e) {
            // SQL Server indisponível: coluna exibirá "—" sem interromper a tela
        }
    ...
    /** @param array{by_cpf: array<string, string>, by_nome_cod: array<string, string>} $map */
    private function lookupLinhaCorporativa(array $row, array $map): ?string
    {
        $cpf = normalize_cpf($row['CPF'] ?? '');
        if (strlen($cpf) === 11 && isset($map['by_cpf'][$cpf])) {
            return $map['by_cpf'][$cpf];
        }
        $key = $this->nomeCodKey(
            (string)($row['NOMECOMPLETO'] ?? ''),
            (string)($row['CODPESSOA'] ?? '')
        );
        return $map['by_nome_cod'][$key] ?? null;
    }

    private function nomeCodKey(string $nome, string $codpessoa): string
    {
        $nomeNorm = mb_strtoupper(trim(preg_replace('/\s+/u', ' ', $nome) ?? ''), 'UTF-8');
        return $nomeNorm . "\0" . trim($codpessoa);
    }

    private function linhaCorporativaFromSql(): string
    {
        return "
    ...
    /** Formata telefone BR: (XX) XXXXX-XXXX (11 dígitos) ou (XX) XXXX-XXXX (10 dígitos). */
    private function formatTelefoneBr(string $numero): string
    {
    ...
    private function formatLinhaCorporativaPlain(array $numeros): string
    {
        ...
        $first = $this->formatTelefoneBr($numeros[0]);
    ...
    private function buildColabConditions(array $fv): array
    {
        ...
        if (($fv['f_linha'] ?? '') !== '') {
            $qLinha = trim((string)$fv['f_linha']);
            $digits = preg_replace('/\D+/', '', $qLinha) ?? '';
            $matchedCpfs = [];
            $matchedNomeCod = [];
            try {
                $sqlSrv = \Database\Connection::get();
                $like = $digits !== '' ? '%' . $digits . '%' : '%' . $qLinha . '%';
                $stLinha = $sqlSrv->prepare("
                    SELECT e.colaborador_cpf, e.colaborador_codpessoa, e.colaborador_nome
                    " . $this->linhaCorporativaFromSql() . "
                    WHERE  e.status IN ('ativo', 'reservado')
                      AND  COALESCE(l_direct.numero_linha, l_linked.numero_linha) LIKE :q
                ");
                $stLinha->execute([':q' => $like]);
                foreach ($stLinha->fetchAll(\PDO::FETCH_ASSOC) as $row) {
                    $cpf = normalize_cpf($row['colaborador_cpf'] ?? '');
                    if (strlen($cpf) === 11) {
                        $matchedCpfs[$cpf] = true;
                    } else {
                        $cod = trim((string)($row['colaborador_codpessoa'] ?? ''));
                        $nome = trim((string)($row['colaborador_nome'] ?? ''));
                        if ($cod !== '' && $nome !== '') {
                            $matchedNomeCod[] = ['cod' => $cod, 'nome' => $nome];
                        }
                    }
                }
            } catch (\Throwable $e) { /* SQL Server indisponível → sem resultados */ }
            ...
            $orParts = [];
            $matchedCpfs = array_keys($matchedCpfs);
            if (!empty($matchedCpfs)) {
                $cpfPh = implode(',', array_map(fn($i) => ":linha_cpf{$i}", array_keys($matchedCpfs)));
                $orParts[] = "CPF IN ({$cpfPh})";
                foreach ($matchedCpfs as $i => $cpf) {
                    $binds[":linha_cpf{$i}"] = $cpf;
                }
            }
            foreach ($matchedNomeCod as $i => $pair) {
                $orParts[] = "(TO_CHAR(CODPESSOA) = :linha_cod{$i} AND UPPER(NOMECOMPLETO) = UPPER(:linha_nome{$i}))";
                $binds[":linha_cod{$i}"] = $pair['cod'];
                $binds[":linha_nome{$i}"] = $pair['nome'];
            }
    ...
    private function exportColaboradoresCsv(): void
    {
        ...
        $cols = [
            ...
            'linha_corporativa'      => ['label' => 'Linha corporativa',       'sortable' => true,  'filter' => 'text',   'param' => 'f_linha', 'render' => fn($v) => $v],
    ...
    public function showColaborador(string $id): void
    {
        \Security\Auth::requireAuth();
        \Security\Permission::require('hr.colaboradores', 1);

        $data = $this->loadColaboradorShowData($id);
        ...
        if (($_GET['export'] ?? '') === 'pdf') {
            $this->exportColaboradorPdf($id, $data);
            return;
        }
    ...
    private function loadColaboradorShowData(string $id): array
    {
        $colaborador = null;
        $erro        = null;
        $cpfDigits   = normalize_cpf($id);
        $byCpf       = strlen($cpfDigits) === 11;
        ...
        $itens = [];
        if ($colaborador) {
            $cpf = normalize_cpf($colaborador['CPF'] ?? '');
            $cod = (string)($colaborador['CODPESSOA'] ?? '');
            $nome = (string)($colaborador['NOMECOMPLETO'] ?? '');
            try {
                ...
                $stmt2 = $pdo2->prepare("
                    ...
                    WHERE  e.status IN ('ativo', 'reservado')
                      AND  (
                            (e.colaborador_cpf IS NOT NULL AND e.colaborador_cpf = :cpf)
                            OR (
                                (e.colaborador_cpf IS NULL OR LTRIM(RTRIM(e.colaborador_cpf)) = '')
                                AND e.colaborador_codpessoa = :cod
                                AND UPPER(LTRIM(RTRIM(e.colaborador_nome))) = UPPER(LTRIM(RTRIM(:nome)))
                            )
                      )
                    ORDER BY e.data_entrega DESC
                ");
                $stmt2->execute([
                    ':cpf'  => strlen($cpf) === 11 ? $cpf : '',
                    ':cod'  => $cod,
                    ':nome' => $nome,
                ]);
    ...
    private function exportColaboradorPdf(string $id, array $data): void
    {
        if ($data['erro']) {
            http_response_code(500);
        ...
        $slug = normalize_cpf($c['CPF'] ?? '') ?: (string)$id;
        $nomeArquivo = 'colaborador_' . preg_replace('/[^A-Za-z0-9_-]+/', '_', $slug) . '.pdf';
