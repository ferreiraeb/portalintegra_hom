...
class DrillingGetmanController
{
    ...
    private const DEF_FI_LE_TXT = '1,7%';
    private const DEF_MK_GT_TXT = '30%';
    private const DEF_FI_GT_TXT = '1,7%';
    ...
    public function index()
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['arquivo'])) {
            try {
                ...
                foreach ($sysByRef as $ref => $p) {
                    if (!isset($plan[$ref])) { $soSistema[] = $p; continue; }
                    $valorUS = (float)($plan[$ref]['Valor'] ?? 0);
                ...
                foreach ($plan as $ref => $payload) {
                    if (!isset($sysByRef[$ref])) {
                        $soPlanilha[] = [
                            'ProdutoReferencia'  => (string)$ref,
...
Prod AS (
    SELECT
      PM.Produto_Codigo,
      COALESCE(PM.ProdutoMarca_ReferenciaAlfanumerico, PM.ProdutoMarca_Referencia) AS Produto_Referencia,
