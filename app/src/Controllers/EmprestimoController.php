...
class EmprestimoController
{
    ...
    public function index(): void
    {
        ...
        $cols = [
            ...
            'data_devolucao' => [
                ...
                'filter'   => 'text',
                'param'    => 'f_data_dev',
                'sql_col'  => 'CONVERT(NVARCHAR(10), e.data_devolucao, 23)',
            ],
            'status' => [
                'label'    => 'Status',
                'sortable' => true,
        ...
        if ($fColab !== '') {
            $extra['(e.colaborador_nome LIKE :f_colab OR e.colaborador_codpessoa LIKE :f_colab2 OR e.colaborador_cpf LIKE :f_colab3)'] = [
                ':f_colab'  => '%' . $fColab . '%',
                ':f_colab2' => '%' . $fColab . '%',
                ':f_colab3' => '%' . $fColab . '%',
            ];
        ...
        $sql = "SELECT e.id, e.item_id, e.quantidade, e.colaborador_codpessoa, e.colaborador_cpf, e.colaborador_nome,
    ...
    public function store(): void
    {
        ...
        try {
            ...
            if ($locked && $data['item_id']) {
                redirect('itens/' . (int)$data['item_id']);
            }
    ...
    /** GET ?export=1 — exporta os empréstimos filtrados como CSV. */
    private function exportCsv(): void
    {
        ...
        $sql = "SELECT e.colaborador_codpessoa, e.quantidade, e.data_entrega,
        ...
        $exportCols = [
            ...
            'colaborador_codpessoa'   => 'Cód. Colaborador',
            'quantidade'              => 'Qtd',
            'data_entrega'            => 'Data Entrega',
            'data_prevista_devolucao' => 'Prev. Devolução',
            'data_devolucao'          => 'Data Devolução',
            'status'                  => 'Status',
        ];
        $dateCols = ['data_entrega', 'data_prevista_devolucao', 'data_devolucao'];
    ...
    private function collectPost(): array
    {
        ...
        return [
            ...
            'colaborador_cpf'         => normalize_cpf($_POST['colaborador_cpf'] ?? ''),
    ...
    private function validate(array $data, string $tipo): ?string
    {
        ...
        if (strlen(normalize_cpf($data['colaborador_cpf'] ?? '')) !== 11) {
            return 'Selecione o colaborador na lista (CPF obrigatório).';
        }
