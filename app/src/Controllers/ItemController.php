...
class ItemController
{
    ...
    public function indexByTipo(int $tipoId): void
    {
        ...
        render_page('itens/index.php', [
            ...
            'tipo'         => $tipo,
            'listaUrl'     => $listaUrl,
            'retornoLista' => $this->buildListaRetorno($listaUrl, $lt),
            'lt'           => $lt,
            'total'        => $listing['total'],
            'from'         => $listing['from'],
            'to'           => $listing['to'],
    ...
    public function show(int $id): void
    {
        ...
        render_page('itens/show.php', [
            ...
            'listaUrl'        => $this->resolveListaUrl($tipo),
        ]);
    }
    ...
    private function collectPostData(): array
    {
        return [
            ...
            'cnpj_empresa'       => trim($_POST['cnpj_empresa'] ?? '') ?: null,
            'IdLinha'            => $_POST['IdLinha'] !== '' ? (int)$_POST['IdLinha'] : null,
            'data_contrato'      => trim($_POST['data_contrato'] ?? '') ?: null,
            'duracao'            => $_POST['duracao'] !== '' ? (int)$_POST['duracao'] : null,
            'fim_contrato'       => trim($_POST['fim_contrato'] ?? '') ?: null,
            'fidelizacao'        => trim($_POST['fidelizacao'] ?? '') ?: null,
    ...
    private function editFormContext(int $id, array $item, ?TipoItem $tipoFixo): array
    {
        ...
        return [
            'tiposPorCategoria' => $this->tiposAgrupadosPorCategoria(2),
            'linhasDisponiveis' => $this->linhasDisponiveisParaSelect($id),
            'listaUrl'          => $this->resolveListaUrl($tipoFixo),
            'tipoFixo'          => $tipoFixo,
    ...
    /** URL relativa da listagem do tipo (com filtros/página quando informado via ?retorno=). */
    private function resolveListaUrl(?TipoItem $tipo): ?string
    {
        $retorno = trim((string)($_GET['retorno'] ?? $_POST['retorno'] ?? ''));
        if ($retorno !== '' && preg_match('#^itens/tipo/\d+(?:\?[^\#]*)?$#', $retorno)) {
            return $retorno;
        }

        return $tipo ? 'itens/tipo/' . $tipo->id : null;
    }

    private function buildListaRetorno(string $listaUrl, \Support\ListTable $lt): string
    {
        $params = array_merge(
            [
                'sort'     => $lt->getSort(),
                'dir'      => $lt->getDir(),
                'per_page' => $lt->getPerPage(),
                'page'     => $lt->getPage(),
            ],
            $lt->filterQueryParams()
        );
        $params = array_filter($params, static fn($v) => $v !== null);

        return $params ? $listaUrl . '?' . http_build_query($params) : $listaUrl;
    }
    ...
    private function getItemLoanState(int $itemId, array $item): array
    {
        $pdo = Connection::get();
        $stmt = $pdo->prepare("
            SELECT e.id, e.status, e.colaborador_nome, e.colaborador_codpessoa, e.colaborador_cpf,
