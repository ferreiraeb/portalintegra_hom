...
class MiningBoartController
{
    ...
    public function index()
    {
        ...
        if ($dealerDb === '') {
            return render_page('mining/tabela_boart.php', [
        ...
        if (($_GET['update'] ?? '') === '1') {
            ...
            redirect('mining/tabela-boart'); return;
        }
        ...
        // Markup e Fator: lê do POST; cai nos defaults do tipo quando não enviados
        $defaultParams = $this->paramsBoart($tipoTabela);
        $mkTxt = (string)($_POST['markup'] ?? '');
        $fiTxt = (string)($_POST['fator']  ?? '');
        $mkFrac = $mkTxt !== '' ? $this->percentToFraction($mkTxt, $defaultParams['le']['mk'])
                                : $defaultParams['le']['mk'];
        $fiFrac = $fiTxt !== '' ? $this->percentToFraction($fiTxt, $defaultParams['le']['fi'])
                                : $defaultParams['le']['fi'];
        $tipoLabel = $defaultParams['le']['tipo'];

        // Textos para repopular os inputs na view
        $uiMk = number_format($mkFrac * 100, 2, ',', '.') . '%';
        $uiFi = number_format($fiFrac * 100, 2, ',', '.') . '%';
        ...
        if ($_SERVER['REQUEST_METHOD']==='POST' && isset($_FILES['arquivo'])) {
            ...
            try {
                ...
                // 3) Cruzamento + cálculos com taxa única (markup/fator fornecidos pelo usuário)
                $rows=[]; $soSistema=[]; $soPlanilha=[];
                ...
                foreach ($commons as $kn) {
                    ...
                    // taxa única — sem distinção de faixa de valor
        ...
        render_page('mining/tabela_boart.php', [
            'erro'       => $erro,
            'rows'       => $rows,
            'soSistema'  => $soSistema,
   ...
   // private function toNumber(string $s, float $def=0.0): float { /* ... reuso do Getman ... */ }
    ...
    /** "30%" → 0.30 · "1,7%" → 0.017 · bare number >1 also treated as percent (40 → 0.40) */
...
OPTION (RECOMPILE);
    ...
    {
        $tab = strtolower(trim((string)($_GET['tab'] ?? 'match')));

        switch ($tab) {
            case 'sys':
                $dados    = $_SESSION['boart_sysOnly'] ?? [];
                $filename = 'boart_dealernet.csv';
                break;
            case 'plan':
                $dados    = $_SESSION['boart_xlsxOnly'] ?? [];
                $filename = 'boart_planilha.csv';
                break;
            default:
                $dados    = $_SESSION['boart_result'] ?? [];
                $filename = 'boart_result.csv';
                break;
        }
