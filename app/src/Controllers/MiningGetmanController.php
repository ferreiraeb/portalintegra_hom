...
class MiningGetmanController
{
    ...
    public function index()
    {
        ...
        if ($dealerDb === '') {
            return render_page('mining/tabela_getman.php', [
		...
		if (($_GET['update'] ?? '') === '1') {
			...
			redirect('mining/tabela-getman'); // volta para a tela, onde exibiremos o alerta
			return;
		}
		// AJAX: atualização de preços em chunks (novo endpoint para o toast/modal)
        ...
        render_page('mining/tabela_getman.php', [
            'erro'      => $erro,
            'rows'      => $rows,
            'soSistema' => $soSistema,
    ...
    /** GET ?export=1[&tab=match|sys|plan] */
    private function exportCsv(): void
    {
        ...
        switch ($tab) {
            case 'sys':
                $dados    = $_SESSION['getman_sysOnly'] ?? [];
                $filename = 'getman_dealernet.csv';
                break;
            case 'plan':
                $dados    = $_SESSION['getman_xlsxOnly'] ?? [];
                $filename = 'getman_planilha.csv';
                break;
            default:
                $dados    = $_SESSION['getman_result'] ?? [];
                $filename = 'getman_result.csv';
                break;
        }
