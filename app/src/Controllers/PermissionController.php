...
class PermissionController {
   ...
   public function edit() {
		...
		$newTabelaGetman = isset($_POST['perm_drilling_tabela_getman']) ? (int)$_POST['perm_drilling_tabela_getman'] : 0; // (NOVO)

		$toSave = [
			'users.manage'         => max(0, min(2, $newUsersManage)),
			'drilling.analise_di'  => max(0, min(2, $newDrillingDI)),
			'drilling.tabela_getman' => max(0, min(2, $newTabelaGetman)), // (NOVO)
		];
