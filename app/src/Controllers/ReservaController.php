...
class ReservaController
{
    ...
    public function index(): void
    {
        ...
        $extra  = '';
        if ($filtroColab !== '') {
            $extra    = "AND (e.colaborador_nome LIKE ? OR e.colaborador_codpessoa LIKE ? OR e.colaborador_cpf LIKE ?)";
            $params[] = "%{$filtroColab}%";
            $params[] = "%{$filtroColab}%";
            $params[] = "%{$filtroColab}%";
        }
        ...
        $stmt = $this->pdo->prepare("
            SELECT e.id, e.item_id, e.quantidade, e.colaborador_codpessoa, e.colaborador_cpf, e.colaborador_nome,
    ...
    public function store(): void
    {
        ...
        if (!$data['item_id'] || !$data['colaborador_codpessoa'] || strlen($data['colaborador_cpf']) !== 11 || !$data['colaborador_nome'] || !$data['data_entrega']) {
            render_page('reservas/form.php', [
                'modo'             => 'criar',
                'reserva'          => $data,
        ...
        $emp->colaborador_cpf         = $data['colaborador_cpf'] !== '' ? $data['colaborador_cpf'] : null;
