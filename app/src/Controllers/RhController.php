...
class RhController
{
        ...
        try {
            ...
            if ($useBcp) {
                ...
                try {
                    ...
                    // Snapshot é só acelerador de export; a sincronização Oracle→SQL segue.
                    $csvPartPath = null;
                    $csvFp = null;
                }
            } else {
            ...
            if (!empty($tsvPath) && is_file($tsvPath . '.fmt')) {
                @unlink($tsvPath . '.fmt');
            }
        }
    }
    ...
    private function ajaxRefreshStart(): void
    {
        ...
        try {
            ...
            $logDir = $this->tryStorageDir();
            $log = ($logDir ?: sys_get_temp_dir()) . '/worker_last.log';
        ...
        if (!$hasFilters) {
            try {
                ...
                if (is_file($snap) && filesize($snap) > 10) {
                    $this->repairSnapshotCsvShift($snap, $syncId);
                }
                if (!is_file($snap) || filesize($snap) < 10) {
                    try {
                        $this->buildSnapshotCsvBcp($syncId, $columns);
                ...
                // storage/rh indisponível — exporta direto do SQL Server.
            }
        }
    ...
    private function snapshotCsvPath(string $syncId): string
    {
        ...
        $dir = $this->tryStorageDir();
        if ($dir === null) {
            throw new \RuntimeException('Diretório storage/rh indisponível para snapshot CSV.');
        }
        return $dir . '/composicao_' . $safe . '.csv';
    }
    ...
    private function purgeOldSnapshotCsvs(string $keepSyncId): void
    {
        ...
        foreach (glob($dir . '/composicao_*.csv') ?: [] as $f) {
            if (is_file($f) && realpath($f) !== realpath($keep)) {
                @unlink($f);
            }
        }
        foreach (glob($dir . '/composicao_*.csv.part') ?: [] as $f) {
            @unlink($f);
        }
    }
    ...
    /**
     * Gera snapshot CSV via bcp queryout (rápido). Usado se o arquivo ainda não existe
     * (cache antigo) ou no fallback sem dual-write.
     *
     * Exporta só as colunas de negócio — SyncId/RowNum ficam de fora.
     */
    private function buildSnapshotCsvBcp(string $syncId, array $columns): void
    {
        ...
        $guid = preg_replace('/[^a-f0-9-]/i', '', $syncId);
        if (!$this->isGuidString($guid)) {
            throw new \RuntimeException('SyncId inválido para exportação.');
        }
        $dbSafe = str_replace([']', '.'], '', $database);
        $selectSql = $this->selectListSql($columns ?: $this->dataColumnNames());
        $query = "SELECT {$selectSql} FROM [{$dbSafe}].dbo.RH_ComposicaoRemuneracao"
            . " WHERE SyncId = CAST('{$guid}' AS uniqueidentifier) ORDER BY [RowNum]";
        ...
        if ($in) {
            $first = fgets($in);
            if ($first !== false) {
                $line = $this->maybeStripSyncIdRowNum($first, $guid);
                fwrite($fp, $line);
                if ($line !== $first) {
                    while (($row = fgets($in)) !== false) {
                        fwrite($fp, $this->csvStripLeadingFields($row, 2));
                    }
                } else {
    ...
    /** Snapshot antigo gerado via view BCP (SyncId + RowNum nas linhas de dados). */
    private function repairSnapshotCsvShift(string $path, string $syncId): void
    {
        $guid = preg_replace('/[^a-f0-9-]/i', '', $syncId);
        if (!$this->isGuidString($guid) || !is_file($path)) {
            return;
        }
        $fp = fopen($path, 'rb');
        if ($fp === false) {
        ...
        if ($this->maybeStripSyncIdRowNum($first, $guid) === $first) {
            return;
        }
        ...
        $hdr = fgets($in);
        fwrite($out, $hdr === false ? '' : $hdr);
        while (($row = fgets($in)) !== false) {
            fwrite($out, $this->csvStripLeadingFields($row, 2));
        }
        fclose($in);
        fclose($out);
        if (!@rename($tmp, $path)) {
    ...
    private function maybeStripSyncIdRowNum(string $line, string $guid): string
    {
        $first = strstr($line, ';', true);
        if ($first === false) {
            return $line;
        }
        $norm = trim($first, " \t\r\n{}");
        if (strcasecmp($norm, $guid) !== 0) {
            return $line;
        }
        return $this->csvStripLeadingFields($line, 2);
    }

    private function csvStripLeadingFields(string $line, int $n): string
    {
        $pos = 0;
        for ($i = 0; $i < $n; $i++) {
            $semi = strpos($line, ';', $pos);
            if ($semi === false) {
                return $line;
            }
            $pos = $semi + 1;
    ...
    private function isGuidString(string $s): bool
    {
    ...
    private function runBcpLoad(string $bcpBin, string $tsvPath, string $syncId, int $rowCount): void
    {
        ...
        $fmtFile = $tsvPath . '.fmt';
        $this->writeBcpInFormatFile($fmtFile);
        ...
        $cmd = [
            $bcpBin,
            $database . '.dbo.RH_ComposicaoRemuneracao',
            'in',
            $tsvPath,
            '-f', $fmtFile,
            '-S', $server,
...
");
    ...
    /**
     * Format file do bcp in: mapeia SyncId+RowNum+colunas de negócio,
     * pulando Id (IDENTITY) e CreatedAt (DEFAULT). Evita CREATE VIEW em runtime.
     */
    private function writeBcpInFormatFile(string $path): void
    {
        $names = array_merge(['SyncId', 'RowNum'], $this->dataColumnNames());
        $n = count($names);
        $out = "14.0\r\n{$n}\r\n";
        foreach ($names as $i => $name) {
            ...
            $term = ($i === $n - 1) ? '\\n' : '\\t';
            $out .= sprintf(
                "%d\tSQLCHAR\t0\t0\t\"%s\"\t%d\t%s\tSQL_Latin1_General_CP1_CI_AS\r\n",
                $hostId,
                $term,
                $serverCol,
                $name
        ...
        if (file_put_contents($path, $out) === false) {
            throw new \RuntimeException('Não foi possível gravar o format file do bcp.');
        }
    }
    ...
    /** Garante JSON limpo mesmo com display_errors=On (homologação). */
    ...
    private function tryStorageDir(): ?string
    {
        try {
    ...
    {
        $dir = dirname(__DIR__, 2) . DIRECTORY_SEPARATOR . 'storage' . DIRECTORY_SEPARATOR . 'rh';
        if (!is_dir($dir)) {
            if (!@mkdir($dir, 0775, true) && !is_dir($dir)) {
                throw new \RuntimeException(
                    'Sem permissão para criar storage/rh. No container: '
                    . 'mkdir -p /var/www/html/storage/rh && chown -R www-data:www-data /var/www/html/storage && chmod -R 775 /var/www/html/storage'
                );
            }
        }
        if (!is_writable($dir)) {
            throw new \RuntimeException(
                'storage/rh existe, mas o PHP (www-data) não consegue gravar. '
                . 'Ajuste: chown -R www-data:www-data /var/www/html/storage && chmod -R 775 /var/www/html/storage'
            );
        }
        return $dir;
    }
