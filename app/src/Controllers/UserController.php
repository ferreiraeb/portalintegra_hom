...
class UserController {
	public function index() {
		...
		$cols = [
			...
			'is_active'        => ['label' => 'Status',           'sortable' => true,  'filter' => 'select', 'param' => 'f_status',
			                       'options' => ['' => 'Todos', '1' => 'Ativo', '0' => 'Inativo'], 'default' => '1'],
    ...
    if (is_post()) {
        ...
        $auth->updateLocalUser($id, [
            ...
            'is_active' => ((int)($_POST['is_active'] ?? $user['is_active']) === 1) ? 1 : 0,
        ]);
        redirect('users');
    }
    ...
    public function delete() {
        ...
        $blockedMsg = 'O usuário não pode ser excluído por haver dependências relacionadas a ele. Você ainda pode editar o Status dele para Inativo.';
        $id = (int)($_POST['id'] ?? 0);
        $wantsJson = (
        ...
        $json = function (array $payload, int $code = 200) {
        ...
        if ($id <= 0) {
            if ($wantsJson) {
                $json(['ok' => false, 'message' => 'Usuário inválido.'], 400);
                return;
            }
            redirect('users');
            return;
        ...
        if ($auth->userHasBlockingDependencies($id)) {
            if ($wantsJson) {
                $json(['ok' => false, 'blocked' => true, 'message' => $blockedMsg]);
                return;
            }
            $_SESSION['flash_error'] = $blockedMsg;
            redirect('users');
            return;
        }
        ...
        try {
            ...
            $isFk = ((string)$e->getCode() === '23000')
                || (stripos($e->getMessage(), 'REFERENCE constraint') !== false);
            if ($isFk) {
                if ($wantsJson) {
                    $json(['ok' => false, 'blocked' => true, 'message' => $blockedMsg]);
                    return;
		...
		try {
			...
			$upd = $pdo->prepare("
				UPDATE users SET
					...
					origem='ad', is_active=:is_active, last_sync_at=GETDATE(), updated_at=GETDATE()
			...
			// Contas que sumiram do LDAP (excluídas) continuam sendo inativadas localmente.
			...
			echo json_encode([
				'ok'          => true,
				'message'     => "Sincronização concluída: {$total} usuário(s) processado(s) — {$inserted} inserido(s), {$updated} atualizado(s), {$deactivated} inativado(s).",
				'inserted'    => $inserted,
				'updated'     => $updated,
