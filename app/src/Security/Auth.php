...
class Auth {
    ...
    public function findByLogin(string $login) {
        $st = $this->pdo->prepare("SELECT * FROM users WHERE LOWER(login) = :login");
        $st->execute([':login' => strtolower($login)]);
        return $st->fetch();
    }

    public function findByEmail(string $email) {
        $st = $this->pdo->prepare("SELECT * FROM users WHERE LOWER(email) = :email");
        $st->execute([':email' => strtolower($email)]);
        return $st->fetch();
    }

    public function findByUpn(string $upn) {
        $st = $this->pdo->prepare("SELECT * FROM users WHERE LOWER(upn) = :upn");
        $st->execute([':upn' => strtolower($upn)]);
        return $st->fetch();
    }
    ...
    /**
     * Indica se o usuário possui registros em outras tabelas que impedem o DELETE
     * (FKs para users — permissões são apagadas junto e não bloqueiam).
     */
    public function userHasBlockingDependencies(int $id): bool
    {
        $checks = [
            'SELECT TOP 1 1 FROM dbo.org_chart_drafts WHERE user_id = :id',
            'SELECT TOP 1 1 FROM dbo.itens WHERE created_by = :id OR updated_by = :id',
            'SELECT TOP 1 1 FROM dbo.emprestimos WHERE criado_por = :id',
            'SELECT TOP 1 1 FROM dbo.termos_responsabilidade WHERE gerado_por = :id',
        ];
