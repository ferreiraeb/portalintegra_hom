...
class AniversarioService
{
    ...
    public function getAniversariantesDoDia(?\DateTimeInterface $date = null): array
    {
        ...
        $stmt = $pdo->prepare(
            "SELECT DISTINCT CODPESSOA, NOMECOMPLETO, CPF, NASCIMENTO, EMPRESA, UNIDADE, SETOR, {$extraFields}
             FROM " . self::VIEW . "
             WHERE STATUS = 'ATIVO'
    ...
    /**
     * Aniversariantes de sábado e domingo anteriores à segunda-feira
     * da semana de $refDate (o fim de semana que acaba de terminar).
     *
     * @return array<int, array<string, mixed>>
     */
    public function getAniversariantesDoFimDeSemana(?\DateTimeInterface $refDate = null): array
    {
        [$saturday, $sunday] = $this->weekendDatesBeforeMonday($refDate);
        $mmddSat = $saturday->format('m-d');
        $mmddSun = $sunday->format('m-d');

        $colaboradores = $this->fetchByMmddList([$mmddSat, $mmddSun]);
        $orderMap      = [$mmddSat => 0, $mmddSun => 1];

        usort($colaboradores, function (array $a, array $b) use ($orderMap): int {
            $oa = $orderMap[(string)($a['MMDD_ANIV'] ?? '')] ?? 99;
            $ob = $orderMap[(string)($b['MMDD_ANIV'] ?? '')] ?? 99;
            if ($oa !== $ob) {
                return $oa <=> $ob;
            }
        ...
        $colaboradores = array_map(function (array $row) use ($saturday, $sunday, $mmddSat): array {
            $isSat = (string)($row['MMDD_ANIV'] ?? '') === $mmddSat;
            $dia   = $isSat ? $saturday : $sunday;
            $row['DIA_SEMANA_LABEL'] = $isSat ? 'Sábado' : 'Domingo';
            $row['DATA_ANIV_LABEL']  = $dia->format('d/m');
            return $row;
        }, $colaboradores);
    ...
    /**
     * Sábado e domingo imediatamente anteriores à segunda-feira da semana de $refDate.
     *
     * @return array{0: \DateTimeImmutable, 1: \DateTimeImmutable}
     */
    public function weekendDatesBeforeMonday(?\DateTimeInterface $refDate = null): array
    {
        $ref    = \DateTimeImmutable::createFromInterface($refDate ?? new \DateTimeImmutable('today'));
        $monday = $ref->modify('-' . ((int)$ref->format('N') - 1) . ' days');

        return [$monday->modify('-2 days'), $monday->modify('-1 day')];
    }
