...
class BirthdayEmailService
{
    ...
    private const DEFAULT_IMAGES = [
        ...
        'weekend_head'    => 'public/assets/img/aniversariantes_fim_semana_am_head.png',
        'weekend_foot'    => 'public/assets/img/aniversariantes_dia_am_foot.png',
    ];
     ...
     * Layout de grupo, com imagens próprias (weekend_head / weekend_foot).
     *
     * @param array<int, array<string, mixed>> $aniversariantes
     * @param string[]|null                      $recipientsOverride
     */
    public function sendWeekendGroupEmail(
        ...
        $images = $this->inlineImages('weekend');
    ...
    private function formatGrupoDetalhes(array $row, bool $incluirDiaSemana = false): string
    {
        ...
        $local   = implode(' - ', array_filter([$cargo, $unidade], static fn(string $v) => $v !== ''));
        if ($local !== '') {
            $partes[] = $local;
        }
    ...
    /**
     * @param  'group'|'individual'|'weekend' $kind
