...
class LdapService {
    ...
    /**
     * Busca no AD um usuário pelo atributo mail.
    ...
    public function findUserByMail(string $email): ?array {
        ...
        try {
            ...
            error_log('[LDAP] findUserByMail – bind falhou: ' . $e->getMessage());
            return null;
        }

        $filter = '(&(objectCategory=person)(objectClass=user)(mail=' . ldap_escape($email, '', LDAP_ESCAPE_FILTER) . '))';
        ...
        if (($entries['count'] ?? 0) === 0) return null;
