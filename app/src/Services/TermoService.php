...
class TermoService
{
    ...
    public function gerar(string $codpessoa, string $nome, array $emprestimoIds, ?int $geradoPor = null): int
    {
        ...
        try {
            ...
            $termo->colaborador_cpf       = $this->cpfFromEmprestimos($emprestimoIds);
     ...
     * @return array[]
     */
    public function findByColaborador(string $codpessoa, ?string $cpf = null): array
    {
    ...
    private function cpfFromEmprestimos(array $emprestimoIds): ?string
    {
        foreach ($emprestimoIds as $empId) {
            $emp = Emprestimo::find((int)$empId);
            $digits = normalize_cpf($emp->colaborador_cpf ?? '');
            if (strlen($digits) === 11) {
                return $digits;
            }
        }
