...
class ListTable
{
    ...
    public function readRequest(
        ...
        $this->perPage = in_array($perPage, $this->perPageOptions, true) ? $perPage : $defaultPerPage;
    ...
    /**
     * Params de filtro para URL/export.
     * Mantém '' quando sobrescreve um default não-vazio (ex.: "Todos" com default ATIVO)
     * e preserva '0' (ex.: status Inativo em usuários).
     */
    public function filterQueryParams(): array
    {
        $params = [];
        foreach ($this->cols as $col) {
            if (empty($col['param']) || !isset($col['filter']) || $col['filter'] === null) {
                continue;
            }
            $param   = $col['param'];
            $val     = $this->filterValues[$param] ?? ($col['default'] ?? '');
            $default = $col['default'] ?? '';

            if ($val !== null && $val !== '') {
                $params[$param] = $val;
            } elseif ((string)$val === '' && $default !== '') {
                // "Todos" explícito — precisa ir na URL senão o default volta no próximo request
                $params[$param] = '';
            }
        }
        return $params;
    ...
    public function url(array $overrides = []): string
    {
        ...
        $params = array_merge($params, $this->filterQueryParams(), $overrides);

        // Remove só null; mantém '' (ex.: "Todos" com default) e '0' (ex.: Inativo)
    ...
    public function buildWhere(array $extra = []): array
    {
        ...
        foreach ($this->cols as $key => $col) {
            if (empty($col['param']) || !isset($col['filter']) || $col['filter'] === null) continue;
            if (!empty($col['skip_where'])) continue;
            $param = $col['param'];
