...
if (!function_exists('format_date_br')) {
    /** Formata data ISO (Y-m-d) ou timestamp para dd/mm/aaaa. */
    function format_date_br(mixed $value, string $fallback = '—'): string
    {
        if ($value === null || $value === '') {
            return $fallback;
        }
...
if (!function_exists('format_item_list_cell')) {
    /**
     * Formata valor de célula na listagem de itens por tipo.
     */
    function format_item_list_cell(string $field, mixed $value): string
    {
        if ($value === null || $value === '') {
            return '—';
        }
        if ($field === 'custo_mensal') {
            return 'R$ ' . number_format((float)$value, 2, ',', '.');
        }
        if (in_array($field, ['data_contratacao', 'data_vencimento'], true)) {
            return format_date_br($value);
        }
        if ($field === 'status_linha') {
            $labels = \Models\ItemLinhaTelefonica::statusLinhaLabels();
            return e($labels[(string)$value] ?? (string)$value);
        }
...
if (!function_exists('dual_listbox')) {
    /**
     * Renderiza o widget dual listbox (views/components/_dual_listbox.php).
     *
     * @param array{
     *   id: string,
     *   label?: string|null,
     *   leftLabel: string,
     *   rightLabel: string,
     *   left: array<int, array{value: string|int, text: string, attrs?: array<string, string>}>,
     *   right: array<int, array{value: string|int, text: string, attrs?: array<string, string>}>,
     *   selectedName: string,
     *   metaNames?: array<string, string>,
     ...
     * } $config
     */
    function dual_listbox(array $config): void
    {
        $dlbId           = $config['id'];
        $dlbLabel        = $config['label'] ?? null;
        $dlbLeftLabel    = $config['leftLabel'];
        $dlbRightLabel   = $config['rightLabel'];
        $dlbLeft         = $config['left'];
        $dlbRight        = $config['right'];
        $dlbSelectedName = $config['selectedName'];
        $dlbMetaNames    = $config['metaNames'] ?? [];
        $dlbContextId    = $config['contextId'] ?? '';
        $dlbFormId       = $config['formId'] ?? null;
        $dlbSize         = $config['size'] ?? 12;
        $dlbFilter       = $config['filter'] ?? true;

        require __DIR__ . '/../../views/components/_dual_listbox.php';
    }
}

if (!function_exists('normalize_cpf')) {
    /** Dígitos do CPF (11) ou string vazia. */
    function normalize_cpf(mixed $cpf): string
    {
        return preg_replace('/\D+/', '', (string)$cpf) ?? '';
    }
}

if (!function_exists('colaborador_url')) {
    /**
     * URL do perfil do colaborador. Prefere CPF (identidade real);
     * CODPESSOA só como fallback legado (não é único entre empresas).
     */
    function colaborador_url(mixed $cpf, mixed $codpessoaFallback = null): string
    {
        $cpfDigits = normalize_cpf($cpf);
        if (strlen($cpfDigits) === 11) {
            return base_url('colaboradores/' . urlencode($cpfDigits));
        }
        $cod = trim((string)($codpessoaFallback ?? ''));
        if ($cod !== '') {
            return base_url('colaboradores/' . urlencode($cod));
        }
        return base_url('colaboradores');
    }
}
