  ...
  (function () {
    ...
    document.querySelectorAll('a[href*="action=export"]').forEach(function (link) {
      link.addEventListener('click', function (e) {
        ...
        // Token único por clique para identificar este download no cookie
        ...
        // Barra de progresso, cresce até ~90 % enquanto espera
        ...
        // Dispara o download — navegação permanece na página pois a resposta é attachment
        window.location.href = url.toString();

        // Polling do cookie a cada 500 ms
        // O servidor seta mf_csv_ready={token} logo após o fetchAll(), antes de começar a stremar
        var pollTimer = setInterval(function () {
          var found = document.cookie.split(';').some(function (c) {
            return c.trim() === 'mf_csv_ready=' + token;
          });
          if (!found) return;
          ...
          // Remove o cookie do lado do cliente
          document.cookie = 'mf_csv_ready=; Max-Age=0; path=/; SameSite=Lax';

          // Barra 100 % + mensagem de sucesso
          PortalModal.update(exportBody(
            100,
          ...
          setTimeout(function () { PortalModal.hide(); }, 1800);
        }, 500);
      });
    });
