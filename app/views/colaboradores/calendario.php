...
$isSegunda = (bool)($isSegunda ?? false);
$isFimDeSemana = (bool)($isFimDeSemana ?? false);
$totalFim = count($aniversariantesFim);
...
$renderLista = function (array $rows, string $emptyMsg, callable $badgeFn) use ($erro): void {
    ...
    foreach ($rows as $row) {
        ...
        $url  = colaborador_url($row['CPF'] ?? '', $row['CODPESSOA'] ?? '');
        $badge = $badgeFn($row);
...
<dialog class="cal-email-dialog" id="dlgBirthdayEmail" aria-labelledby="dlgBirthdayEmailTitle">
  <form method="dialog" id="frmBirthdayEmail" class="cal-email-dialog-inner">
    ...
    <div class="cal-email-dialog-body">
      <p class="cal-email-desc small mb-3" id="birthdayEmailDesc">
        Envia o <strong>e-mail de grupo</strong> e os <strong>e-mails individuais</strong>
        de parabéns para os destinatários abaixo, usando os aniversariantes do
        <strong>dia selecionado</strong> no calendário.
      </p>
      <div id="birthdayEmailStatus" class="alert alert-light border py-2 small mb-3">
    ...
    <div class="cal-email-dialog-footer">
      ...
      <button type="submit" class="btn btn-warning btn-sm" id="btnSubmitBirthdayEmail" disabled>
...
(function () {
  ...
  var selectedRef = initialRef;
  var lastAnivData = initialRef ? {
    ref: initialRef,
    diaLabel: <?= json_encode($diaLabel) ?>,
    diaCount: <?= (int)$totalDia ?>,
    fimCount: <?= (int)$totalFim ?>,
    isSegunda: <?= $isSegunda ? 'true' : 'false' ?>,
    isFimDeSemana: <?= $isFimDeSemana ? 'true' : 'false' ?>
  } : null;
  ...
  function renderAnivListHtml(rows, emptyMsg, badgeFn) {
    ...
    rows.forEach(function (row) {
      ...
      html += '<a href="' + escapeHtml(colabBaseUrl + encodeURIComponent(row.cpf || row.codpessoa)) + '" class="cal-list-name">'
  ...
  function loadAniversariantesForRef(ref) {
    ...
    if (!semBody || !diaBody) return Promise.reject(new Error('Painel do calendário não encontrado.'));
    ...
    return fetch(anivJsonUrl + '?ref=' + encodeURIComponent(ref), {
      ...
      .then(function (res) {
        ...
        lastAnivData = {
          ref: ref,
          diaLabel: res.body.diaLabel || '',
          diaCount: (res.body.dia || []).length,
          fimCount: res.body.fimDeSemanaCount || 0,
          isSegunda: !!res.body.isSegunda,
          isFimDeSemana: !!res.body.isFimDeSemana
        };
  ...
  function initBirthdayEmailDialog() {
    ...
    var descEl   = document.getElementById('birthdayEmailDesc');
    var statusEl = document.getElementById('birthdayEmailStatus');
    ...
    function formatRefBr(ref) {
      var p = String(ref || '').split('-');
      if (p.length !== 3) return ref || '';
      return p[2] + '/' + p[1] + '/' + p[0];
    }

    function getSelectedRef() {
      var el = document.querySelector('.cal-day--selected[data-cal-day]');
      if (el) {
        var dia = parseInt(el.getAttribute('data-cal-day'), 10);
        if (dia) {
          return String(currentAno) + '-' + String(currentMes).padStart(2, '0') + '-' + String(dia).padStart(2, '0');
        }
      }
      return selectedRef;
    }
    ...
    function setStatus(type, html) {
      ...
      if (type === 'light') statusEl.className += ' border';
      statusEl.innerHTML = html;
    }

    function refreshEmailDialog() {
      var ref = getSelectedRef();
      selectedRef = ref;

      if (!ref) {
        if (descEl) {
          descEl.innerHTML = 'Envia o <strong>e-mail de grupo</strong> e os <strong>e-mails individuais</strong> '
            + 'de parabéns para os destinatários abaixo, usando os aniversariantes do '
            + '<strong>dia selecionado</strong> no calendário.';
        }
        setStatus('warning', '<i class="fas fa-exclamation-triangle mr-1"></i>Selecione um dia no calendário.');
        if (submitBtn) submitBtn.disabled = true;
        return;
      }

      var data = lastAnivData && lastAnivData.ref === ref ? lastAnivData : null;
      var dataTxt = formatRefBr(ref);
      var extra = '';
      if (data && data.isSegunda) {
        extra = ' Como a data é uma <strong>segunda</strong>, inclui também o resumo dos aniversariantes de sábado e domingo.';
      } else if (data && data.isFimDeSemana) {
        extra = ' Em sábado e domingo o e-mail de grupo do dia não é enviado — os aniversariantes vão agrupados na segunda.';
      }

      if (descEl) {
        descEl.innerHTML = 'Envia o <strong>e-mail de grupo</strong> e os <strong>e-mails individuais</strong> '
          + 'de parabéns para os destinatários abaixo, usando os aniversariantes de '
          + '<strong>' + escapeHtml(data && data.diaLabel ? data.diaLabel : dataTxt) + '</strong>'
          + ' (' + escapeHtml(dataTxt) + ').' + extra;
      }

      if (!data) {
        setStatus('light', '<i class="fas fa-spinner fa-spin mr-1"></i>Carregando aniversariantes desta data…');
        if (submitBtn) submitBtn.disabled = true;
        return;
      }

      var canSend = data.diaCount > 0 || data.fimCount > 0;
      if (!canSend) {
        setStatus('warning', '<i class="fas fa-exclamation-triangle mr-1"></i>Não há aniversariantes para enviar nesta data.');
        if (submitBtn) submitBtn.disabled = true;
        return;
      }

      var html = '<strong>' + data.diaCount + '</strong> aniversariante'
        + (data.diaCount === 1 ? '' : 's') + ' neste dia.';
      if (data.fimCount > 0) {
        html += '<span class="d-block mt-1"><strong>' + data.fimCount + '</strong> aniversariante'
          + (data.fimCount === 1 ? '' : 's') + ' do fim de semana.</span>';
      }
      setStatus('light', html);
    ...
    openBtn.addEventListener('click', function () {
      ...
      var ready = (lastAnivData && lastAnivData.ref === ref)
        ? Promise.resolve(lastAnivData)
        : loadAniversariantesForRef(ref);
      ...
      ready.then(function () {
        ...
        setStatus('danger', '<i class="fas fa-exclamation-triangle mr-1"></i>'
          + escapeHtml(err.message || 'Falha ao carregar aniversariantes.'));
        if (submitBtn) submitBtn.disabled = true;
      });
    });
    ...
    if (form) {
      form.addEventListener('submit', function (e) {
        ...
        var ref = getSelectedRef();
        if (!ref) {
          showFeedback('danger', 'Selecione um dia no calendário.');
          return;
        }
