...
<section class="content pt-3">
  <div class="container-fluid">
    ...
    <form method="get" action="<?= base_url('colaboradores') ?>" id="frmColabFilter">
      ...
      <div class="card">
        ...
        <div class="card-header d-flex align-items-center">
          ...
          ?>
          <a href="<?= e(base_url('colaboradores') . '?' . http_build_query($exportParams)) ?>"
          ...
          <?php if ($lt->hasFilters()): ?>
            <a href="<?= base_url('colaboradores') ?>" class="btn btn-sm btn-outline-secondary ml-2" title="Limpar filtros">
                    ...
                    <?php foreach ($lt->cols() as $colKey => $col): ?>
                      <td class="<?= e($col['th_class'] ?? '') ?>"><?php
                        $cellHtml = isset($col['render'])
                            ...
                            : e((string)($row[$colKey] ?? ''));
                        if ($colKey === 'NOMECOMPLETO' && (!empty($row['CPF']) || !empty($row['CODPESSOA']))) {
                            echo '<a href="' . colaborador_url($row['CPF'] ?? '', $row['CODPESSOA'] ?? '') . '">'
                                . $cellHtml . '</a>';
                        } else {
                            echo $cellHtml;
                        }
