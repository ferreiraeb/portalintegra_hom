...
$tipoItemId = (int)($tipoFixo?->id ?? $i['tipo_item_id'] ?? 0);
$voltarUrl = $listaUrl
    ? base_url($listaUrl)
    : ($tipoItemId ? base_url('itens/tipo/' . $tipoItemId) : null);
...
$statusLabels    = \Services\ItemService::statusLabels();
$manualStatuses  = \Services\ItemService::manualStatusOptions();
$podeAlterarStatus = ($nivelUsuario ?? 0) >= 3;
$statusAtualForm = $i['status'] ?? 'disponivel';
$statusBloqueado = $statusBloqueado ?? null;
$showEmprestimoCard = $isEdit && $editIsDeterminado && !empty($emprestimosItem);
  ...
  <div class="container-fluid" style="max-width:<?= $containerMax ?>;">
    ...
    <div class="card card-primary">
      ...
      <form method="post" action="<?= $action ?>">
        <?php csrf_field(); ?>
        <?php if ($listaUrl): ?>
        <input type="hidden" name="retorno" value="<?= e($listaUrl) ?>">
        <?php endif; ?>
        <div class="card-body">
          <?php if ($erro): ?>
          ...
          <div class="row">
            <div class="col-md-6">
              <div class="form-group">
                ...
                <?php if ($isEdit && !$podeAlterarStatus): ?>
                  <input type="hidden" name="status" value="<?= e($statusAtualForm) ?>">
                  <input type="text" class="form-control" readonly
                         value="<?= e($statusLabels[$statusAtualForm] ?? $statusAtualForm) ?>">
                  <small class="form-text text-muted">Alteração de status requer permissão Gerenciar.</small>
                <?php elseif ($isEdit && $statusBloqueado !== null): ?>
                  <input type="hidden" name="status" value="<?= e($statusBloqueado) ?>">
                  <select class="form-control" disabled>
                    <option selected><?= e($statusLabels[$statusBloqueado] ?? $statusBloqueado) ?></option>
                  </select>
                  <small class="form-text text-muted">
                    Status controlado pelo <?= $statusBloqueado === 'reservado' ? 'reserva' : 'empréstimo' ?> ativo.
                    Gerencie em <a href="<?= base_url('emprestimos/' . (int)($emprestimosItem[0]['id'] ?? 0)) ?>">Empréstimos</a>.
                  </small>
                <?php else: ?>
                <select name="status" class="form-control" required>
                  <?php foreach ($manualStatuses as $sv => $sl): ?>
                    <option value="<?= $sv ?>" <?= $statusAtualForm === $sv ? 'selected' : '' ?>><?= $sl ?></option>
                  <?php endforeach; ?>
                </select>
                <?php if ($isEdit && $statusAtualForm === 'em_uso'): ?>
                  <small class="form-text text-warning">
                    Item sem empréstimo ativo — selecione o novo status manualmente.
                  </small>
                <?php endif; ?>
                <?php endif; ?>
              </div>
              ...
              <div class="row">
                ...
                <div class="col-md-4 form-group">
                  ...
                  <select name="status_linha" class="form-control">
                    <?php foreach (\Models\ItemLinhaTelefonica::statusLinhaLabels() as $sv => $sl): ?>
                ...
                <div class="col-md-6 form-group">
                  <label>Empresa Faturamento (CNPJ)</label>
                  <input name="cnpj_empresa" class="form-control" maxlength="14" placeholder="Somente números" value="<?= $v('cnpj_empresa') ?>">
                </div>
                <div class="col-md-6 form-group">
                  <label>ID Linha</label>
                  <input name="IdLinha" type="number" min="0" class="form-control" value="<?= $v('IdLinha') ?>">
                </div>
                <div class="col-md-6 form-group">
                  <label>Data Contrato</label>
                  <input name="data_contrato" type="date" class="form-control" value="<?= $v('data_contrato') ?>">
                </div>
                <div class="col-md-6 form-group">
                  <label>Fim Contrato</label>
                  <input name="fim_contrato" type="date" class="form-control" value="<?= $v('fim_contrato') ?>">
                </div>
                <div class="col-md-6 form-group">
                  <label>Duração (Meses)</label>
                  <input name="duracao" type="number" min="0" class="form-control" value="<?= $v('duracao') ?>">
                </div>
                <div class="col-md-6 form-group">
                  <label>Fidelização (Meses)</label>
                  <input name="fidelizacao" class="form-control" maxlength="3" value="<?= $v('fidelizacao') ?>">
                </div>
                <div class="col-md-12 form-group">
                  <label>Observações</label>
                  <textarea name="observacoes" class="form-control" rows="2"><?= $v('observacoes') ?></textarea>
                </div>
              </div>
            </div>
