...
$editTipoId        = (int)($i['tipo_item_id'] ?? 0);
$editTabela        = e($i['tabela_detalhe'] ?? '');
$editIsDeterminado = (int)($i['is_determinado'] ?? 0);
    ...
    <div class="card card-primary">
      ...
      <form method="post" action="<?= $action ?>">
        ...
        <div class="card-footer text-right">
          <a href="<?= base_url('itens') ?>" class="btn btn-secondary">Cancelar</a>
...
(function () {
  ...
  // Inicialização na edição
  <?php if ($isEdit && $editTipoId): ?>
  (function () {
    var opt = selectTipo.querySelector('option[value="<?= $editTipoId ?>"]');
    aplicarTipo(
      <?= $editIsDeterminado ?>,
      '<?= $editTabela ?>',
      opt ? opt.getAttribute('data-is-movel') === '1' : false
    );
  })();
  <?php endif; ?>
})();
