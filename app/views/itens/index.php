<?php
/** @var \Support\ListTable $lt @var array $itens @var \Models\TipoItem $tipo @var string $listaUrl @var string $retornoLista @var int $total @var int $from @var int $to @var bool $canCreate */
$retornoLista = $retornoLista ?? $listaUrl;
$retornoQs = '?retorno=' . urlencode($retornoLista);
$statusLabels = [
            ...
            <?php if (!empty($itens)): ?>
              ...
              <tr>
                ...
                <td class="text-right text-nowrap">
                  <a href="<?= base_url('itens/'.(int)$i['id']) . $retornoQs ?>" class="btn btn-xs btn-outline-secondary">Ver</a>
                  <?php if (($i['nivel_usuario'] ?? 0) >= 2): ?>
                  <a href="<?= base_url('itens/'.(int)$i['id'].'/editar') . $retornoQs ?>" class="btn btn-xs btn-outline-primary">Editar</a>
                  <?php
                ...
                <td>
                  ...
                  <a href="<?= base_url('colaboradores/' . urlencode((string)($i['colaborador_codpessoa'] ?? ''))) ?>">
