...
$tipoItemId = (int)($i['tipo_item_id'] ?? $tipo['id'] ?? 0);
$voltarUrl = $listaUrl
    ? base_url($listaUrl)
    : ($tipoItemId ? base_url('itens/tipo/' . $tipoItemId) : null);
$retornoQs = $listaUrl ? '?retorno=' . urlencode($listaUrl) : '';
$statusLabels  = [
  ...
  <div class="container-fluid" style="max-width:<?= $containerMax ?>;">
    ...
    <div class="card card-primary">
      ...
      <div class="card-footer d-flex justify-content-end align-items-center flex-wrap">
        <?php if ($voltarUrl): ?>
        <a href="<?= $voltarUrl ?>" class="btn btn-secondary mr-2 mb-1">
          <i class="fas fa-arrow-left mr-1"></i>Voltar à lista
        </a>
        <?php endif; ?>
        <?php if (($nivelUsuario ?? 0) >= 2): ?>
        <a href="<?= base_url('itens/'.(int)$i['id'].'/editar') . $retornoQs ?>" class="btn btn-primary mr-2 mb-1">
