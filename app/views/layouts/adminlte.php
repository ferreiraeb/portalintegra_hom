  ...
  <aside class="main-sidebar sidebar-dark-primary elevation-4">
    ...
    <div class="sidebar">
      <nav class="mt-2">
		...
		<?php if ($canRhColabs): ?>
		<?php $rhMenuOpen = starts_with_path('colaboradores', $currentPath); ?>
		<li class="nav-item has-treeview<?= $rhMenuOpen ? ' menu-open' : '' ?>">
		  <a href="<?= base_url('colaboradores') ?>" class="nav-link<?= $rhMenuOpen ? ' active' : '' ?>">
			<i class="nav-icon fas fa-id-badge"></i>
			<p>Colaboradores<i class="right fas fa-angle-left"></i></p>
		  </a>
		  <ul class="nav nav-treeview"<?= $rhMenuOpen ? ' style="display:block;"' : '' ?>>
			<li class="nav-item">
			  <a href="<?= base_url('colaboradores') ?>" class="nav-link<?= is_current('colaboradores', $currentPath) ? ' active' : '' ?>">
				<i class="far fa-circle nav-icon"></i><p>Listar</p>
			  </a>
			</li>
			<li class="nav-item">
			  <a href="<?= base_url('colaboradores/organograma') ?>" class="nav-link<?= is_current('colaboradores/organograma', $currentPath) ? ' active' : '' ?>">
				<i class="far fa-circle nav-icon"></i><p>Organograma</p>
			  </a>
			</li>
			<li class="nav-item">
			  <a href="<?= base_url('colaboradores/calendario') ?>" class="nav-link<?= is_current('colaboradores/calendario', $currentPath) ? ' active' : '' ?>">
				<i class="far fa-circle nav-icon"></i><p>Calendário</p>
			  </a>
			</li>
		  </ul>
		</li>
		...
		<?php $canRhComp = (\Security\Permission::level('rh.composicao_remuneracao') >= 1); ?>
		<?php if ($canRhComp): ?>
		<?php $rhModMenuOpen = starts_with_path('rh', $currentPath); ?>
		<li class="nav-item has-treeview<?= $rhModMenuOpen ? ' menu-open' : '' ?>">
		  <a href="<?= base_url('rh/composicao-remuneracao') ?>" class="nav-link<?= $rhModMenuOpen ? ' active' : '' ?>">
			<i class="nav-icon fas fa-user-tie"></i>
			<p>RH<i class="right fas fa-angle-left"></i></p>
		  </a>
		  <ul class="nav nav-treeview"<?= $rhModMenuOpen ? ' style="display:block;"' : '' ?>>
			<li class="nav-item">
			  <a href="<?= base_url('rh/composicao-remuneracao') ?>" class="nav-link<?= is_current('rh/composicao-remuneracao', $currentPath) ? ' active' : '' ?>">
				<i class="far fa-circle nav-icon"></i><p>Composição Remuneração</p>
			  </a>
			</li>
		  </ul>
		</li>
		...
		<?php
			$cadastroMenuOpen = starts_with_path('itens', $currentPath);
			$emprestimoMenuOpen = starts_with_path('emprestimos', $currentPath)
			                   || starts_with_path('reservas', $currentPath);
			$configMenuOpen   = starts_with_path('categorias', $currentPath)
			                 || starts_with_path('tipos-item', $currentPath);

			// Mostrar Cadastro se o usuário tiver acesso (nivel >= 1) em ao menos uma categoria
			$accessibleAlmox = \Security\AlmoxPermission::accessibleCategoryIds(1);
			$canSeeCadastro  = $accessibleAlmox === null || count($accessibleAlmox) > 0;

			// Tipos de item visíveis no menu (ativo + permissão na categoria)
			$tiposMenu = [];
			if ($canSeeCadastro) {
				foreach (\Models\TipoItem::findAll(['ativo' => 1], 'nome') as $tipoMenu) {
					if ($accessibleAlmox === null || in_array($tipoMenu->categoria_id, $accessibleAlmox, true)) {
						$tiposMenu[] = $tipoMenu;
					}
				}
			}

			// Mostrar Configurações se for admin de almoxarifado ou gestor de usuários
			$canSeeConfig = \Security\Permission::level('users.manage') >= 2
			             || \Security\Permission::level('almoxarifado.manage') >= 2;
		?>
		<?php if ($canSeeCadastro): ?>
		<li class="nav-item has-treeview<?= $cadastroMenuOpen ? ' menu-open' : '' ?>">
		  <a href="#" class="nav-link<?= $cadastroMenuOpen ? ' active' : '' ?>">
			<i class="nav-icon fas fa-box-open"></i>
			<p>Ativos<i class="right fas fa-angle-left"></i></p>
		  </a>
		  <ul class="nav nav-treeview"<?= $cadastroMenuOpen ? ' style="display:block;"' : '' ?>>
			<?php foreach ($tiposMenu as $tipoMenu): ?>
			<li class="nav-item">
			  <a href="<?= base_url('itens/tipo/'.(int)$tipoMenu->id) ?>"
			     class="nav-link<?= is_current('itens/tipo/'.(int)$tipoMenu->id, $currentPath) ? ' active' : '' ?>">
				<i class="far fa-circle nav-icon"></i><p><?= e($tipoMenu->nome) ?></p>
			  </a>
			</li>
			<?php endforeach; ?>
		  </ul>
		...
		<!-- Almoxarifado — EMPRÉSTIMO -->
		<?php if ($canSeeCadastro): ?>
		<li class="nav-item">
		  <a href="<?= base_url('emprestimos') ?>" class="nav-link<?= $emprestimoMenuOpen ? ' active' : '' ?>">
		...
		<?php if ($canSeeConfig): ?>
		<li class="nav-item has-treeview<?= $configMenuOpen ? ' menu-open' : '' ?>">
		  <a href="#" class="nav-link<?= $configMenuOpen ? ' active' : '' ?>">
			<i class="nav-icon fas fa-cog"></i>
			<p>Configurações<i class="right fas fa-angle-left"></i></p>
		  </a>
		  <ul class="nav nav-treeview"<?= $configMenuOpen ? ' style="display:block;"' : '' ?>>
			<li class="nav-item">
			  <a href="<?= base_url('categorias') ?>" class="nav-link<?= starts_with_path('categorias', $currentPath) ? ' active' : '' ?>">
				<i class="far fa-circle nav-icon"></i><p>Categorias</p>
			  </a>
			</li>
			<li class="nav-item">
			  <a href="<?= base_url('tipos-item') ?>" class="nav-link<?= starts_with_path('tipos-item', $currentPath) ? ' active' : '' ?>">
				<i class="far fa-circle nav-icon"></i><p>Tipos de Item</p>
			  </a>
			</li>
		  </ul>
		</li>
...
<!-- ── RH Composição: polling global enquanto worker CLI roda ──────────── -->
<script>
(function () {
  'use strict';

  var LS_KEY      = 'pi_rh_comp_refresh';
  var POLL_MS     = <?= \Controllers\RhController::REFRESH_POLL_INTERVAL_MS ?>;
  var STALE_MS    = 2 * 60 * 60 * 1000;
  var STALE_TICKS = <?= \Controllers\RhController::REFRESH_STALE_TICKS ?>;
  ...
  if (window.rhCompPageActive) return;
  ...
  function tick() {
    ...
    fetch(pending.statusUrl, { cache: 'no-store' })
      .then(function (r) { return r.ok ? r.json() : Promise.reject(r.status); })
      .then(function (j) {
        if (!j || !j.ok) { schedule(); return; }

        if (j.status === 'done') {
          localStorage.removeItem(LS_KEY);
          PortalToast.show('success', 'RH — Sincronização concluída',
            (j.message || 'Concluído.') +
            '<br><small class="text-muted">Pode continuar normalmente.</small>',
            30000);
          nativeNotify('Portal Integra — RH', j.message || 'Composição remuneração atualizada.');
          return;
        }
        if (j.status === 'error') {
          localStorage.removeItem(LS_KEY);
          PortalToast.show('danger', 'RH — Erro',
            (j.error || j.message || 'Ocorreu um erro.') +
            '<br><small class="text-muted">Verifique e tente novamente.</small>');
          nativeNotify('Portal Integra — Erro', j.error || j.message || 'Erro na sincronização.');
          return;
        }
        if (j.status === 'running') {
          ...
          if (workerBusy) {
            ...
            if (++staleTicks >= STALE_TICKS) {
              localStorage.removeItem(LS_KEY);
              PortalToast.show('danger', 'RH — Interrompido',
                'Sem progresso detectado (15 min). A operação pode ter sido interrompida.');
              return;
            }
          } else {
            staleTicks = 0;
          ...
          var pct = parseInt(j.progress, 10) || 0;
          var msg = j.message || 'Processando...';
          PortalToast.update(PortalToast.progressHtml(pct, msg, true));
          schedule();
        } else {
          localStorage.removeItem(LS_KEY);
          PortalToast.hide();
  ...
  PortalToast.show('info', 'RH — Sincronizando',
    PortalToast.progressHtml(0, 'Aguardando progresso...'));
  setTimeout(tick, 1000);
})();
</script>
