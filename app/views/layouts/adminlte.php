		...
		<?php
			...
			<?php if ($canSeeAnaliseDI || $canBoart || $canGetman): ?>
			  <li class="nav-item has-treeview<?= $drillingMenuOpen ? ' menu-open' : '' ?>">
				<a href="<?= base_url('drilling') ?>" class="nav-link<?= $drillingMenuOpen ? ' active' : '' ?>">
				...
				<ul class="nav nav-treeview"<?= $drillingMenuOpen ? ' style="display:block;"' : '' ?>>
				  ...
				  <?php if ($canBoart): ?>
				  <li class="nav-item">
					<a href="<?= base_url('drilling/tabela-boart') ?>" class="nav-link<?= is_current('drilling/tabela-boart', $currentPath) ? ' active' : '' ?>">
...
<!-- ── Massey: polling global de atualização de preços ───────────────────
     ...
     Quando completa, dispara notificação nativa + banner fixo no topo.
──────────────────────────────────────────────────────────────────────── -->
<script>
(function () {
  'use strict';
  ...
  /* ── Toast persistente (mesma aparência do mf-update-notice) ────────── */
  var toast = null;
  ...
  function getToast() {
    ...
    toast.style.cssText =
      ...
      'min-width:280px;max-width:400px;display:block;';
    toast.innerHTML =
      '<div class="card card-outline mb-0 shadow" id="pi-mt-card" ' +
          'style="border-left:4px solid #007bff;">' +
        '<div class="card-header py-2 d-flex align-items-center" style="background:#fff;">' +
          '<span id="pi-mt-icon" class="mr-2">' +
            '<i class="fas fa-sync fa-spin text-primary"></i>' +
          '</span>' +
          '<strong id="pi-mt-title" class="flex-grow-1 small text-uppercase">' +
            'Massey — Atualizando' +
          '</strong>' +
          '<button id="pi-mt-close" type="button" ' +
              'class="btn btn-sm p-0 ml-3 text-secondary" title="Fechar">' +
            '<i class="fas fa-times"></i>' +
          '</button>' +
        '</div>' +
        '<div id="pi-mt-body" class="card-body py-2 px-3 small">Iniciando...</div>' +
      '</div>';
    document.body.appendChild(toast);
    document.getElementById('pi-mt-close').onclick = function () {
  ...
  function setRunning(pct, msg) {
    var t = getToast();
    t.style.display = 'block';
    var card  = document.getElementById('pi-mt-card');
    var icon  = document.getElementById('pi-mt-icon');
    var title = document.getElementById('pi-mt-title');
    var body  = document.getElementById('pi-mt-body');
    if (card)  card.style.borderLeft  = '4px solid #007bff';
    if (icon)  icon.innerHTML  = '<i class="fas fa-sync fa-spin text-primary"></i>';
    if (title) title.textContent = 'Massey — Atualizando';
    if (body)  body.innerHTML   = progressHtml(pct, msg);
  }
  ...
  function setDone(msg) {
    ...
    var title = document.getElementById('pi-mt-title');
    var body  = document.getElementById('pi-mt-body');
    if (card)  card.style.borderLeft  = '4px solid #28a745';
    if (icon)  icon.innerHTML  = '<i class="fas fa-check-circle text-success"></i>';
    if (title) title.textContent = 'Massey — Preços Atualizados';
    if (body)  body.innerHTML   =
      (msg || 'Concluído.') +
      '<br><small class="text-muted">Pode continuar normalmente.</small>';
    setTimeout(function () { if (t) t.style.display = 'none'; }, 30000);
  }
  ...
  function setError(msg) {
    ...
    if (icon)  icon.innerHTML  = '<i class="fas fa-exclamation-circle text-danger"></i>';
    if (title) title.textContent = 'Massey — Erro';
    if (body)  body.innerHTML   =
      (msg || 'Ocorreu um erro.') +
  ...
  function tick() {
    if (window.masseyPageActive) {
      // Página Massey aberta — esconde o toast global e para
        ...
        if (step === 'UPDATE_RUNNING' || step === 'UPDATE_QUEUED') {
          if (j.data.UpdatedAt === lastUpdatedAt) {
            if (++staleTicks >= STALE_TICKS) {
              localStorage.removeItem(LS_KEY);
              setError('Sem progresso detectado. A operação pode ter sido interrompida.');
              return;
            }
          } else {
            staleTicks    = 0;
          ...
          setRunning(pct, j.data.Message || 'Processando...');
          schedule();
        } else {
          localStorage.removeItem(LS_KEY);
          if (toast) toast.style.display = 'none';
  ...
  setRunning(0, 'Aguardando progresso...');
