...
$levels = $levels ?? [
        ...
        'hr.colaboradores'          => 0,
        'hr.organograma_proposta'   => 0,
        'rh.composicao_remuneracao' => 0,
...
<form action="<?= htmlspecialchars($actionUrl, ENT_QUOTES, 'UTF-8') ?>" method="post" id="modalForm">
    ...
    <div class="card mb-3">
        <div class="card-header py-2"><strong>Permissões — Organograma → Proposta de alteração</strong></div>
        <div class="card-body">
            <?php
            $orgPropostaMap = [
                0 => 'Nenhum — só visualiza o organograma (se tiver Colaboradores)',
                1 => 'Proposta — editar líderes, salvar rascunhos e exportar CSV para o RH',
            ];
            foreach ($orgPropostaMap as $val => $label): ?>
                <div class="custom-control custom-radio mb-2">
                    <input type="radio"
                           id="perm_org_prop_<?= (int)$val ?>"
                           name="perm_hr_organograma_proposta"
                           class="custom-control-input"
                           value="<?= (int)$val ?>"
                            <?= ((int)($levels['hr.organograma_proposta'] ?? 0) === (int)$val) ? 'checked' : '' ?>>
                    <label class="custom-control-label" for="perm_org_prop_<?= (int)$val ?>">
                        <strong><?= (int)$val ?>:</strong> <?= htmlspecialchars($label) ?>
                    </label>
                </div>
            <?php endforeach; ?>
            <small class="form-text text-muted">
                Não altera a base oficial do RH — apenas gera propostas/exportações.
            </small>
        </div>
    </div>

    <!-- RH — Composição Remuneração -->
    <div class="card mb-3">
        <div class="card-header py-2"><strong>Permissões — RH → Composição Remuneração</strong></div>
        <div class="card-body">
            <?php foreach ($mapText as $val => $label): ?>
                <div class="custom-control custom-radio mb-2">
                    <input type="radio"
                           id="perm_rh_comp_<?= (int)$val ?>"
                           name="perm_rh_composicao_remuneracao"
                           class="custom-control-input"
                           value="<?= (int)$val ?>"
                            <?= ((int)($levels['rh.composicao_remuneracao'] ?? 0) === (int)$val) ? 'checked' : '' ?>>
                    <label class="custom-control-label" for="perm_rh_comp_<?= (int)$val ?>">
                        <strong><?= (int)$val ?>:</strong> <?= htmlspecialchars($label) ?>
                    </label>
                </div>
            <?php endforeach; ?>
            <small class="form-text text-muted">
                Nível <strong>1 (Leitura)</strong> permite ver o menu, filtrar, exportar CSV e atualizar o cache.
            </small>
        </div>
    </div>
