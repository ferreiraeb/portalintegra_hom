...
<section class="content pt-3">
  <div class="container-fluid">
    ...
    <?php
    $almoxLevelLabels = [
        3 => ['label' => 'Gerenciar', 'badge' => 'badge-danger',   'icon' => 'fas fa-shield-alt', 'row' => 'table-danger'],
        2 => ['label' => 'Editar',    'badge' => 'badge-warning',  'icon' => 'fas fa-pen',         'row' => 'table-warning'],
        1 => ['label' => 'Consultar', 'badge' => 'badge-primary',  'icon' => 'fas fa-eye',         'row' => 'table-primary'],
    ];
    ?>
    <div class="row">
      <?php foreach ($categorias as $cat): ?>
        <?php
          $byLevel = $almoxByPerm[$cat->id] ?? [3 => [], 2 => [], 1 => []];
          $total   = count($byLevel[3]) + count($byLevel[2]) + count($byLevel[1]);
        ?>
        <div class="col-12 col-xl-6 mb-4">
          <div class="card shadow-sm h-100">
            <div class="card-header d-flex align-items-center justify-content-between py-2">
              <div>
                <strong>Almoxarifado — <?= e($cat->nome) ?></strong>
              </div>
              <div class="ml-auto flex-shrink-0 d-flex align-items-center" style="gap:.25rem">
                <span class="badge badge-danger mr-1" title="Gerenciar"><i class="fas fa-shield-alt mr-1"></i><?= count($byLevel[3]) ?></span>
                <span class="badge badge-warning mr-1" title="Editar"><i class="fas fa-pen mr-1"></i><?= count($byLevel[2]) ?></span>
                <span class="badge badge-primary" title="Consultar"><i class="fas fa-eye mr-1"></i><?= count($byLevel[1]) ?></span>
              </div>
            </div>
            <div class="card-body p-0">
              <?php if ($total === 0): ?>
                <div class="p-3 text-center text-muted small">
                  <i class="fas fa-lock mr-1"></i> Nenhum usuário com acesso a esta categoria.
                </div>
              <?php else: ?>
                <table class="table table-sm mb-0">
                  <thead class="thead-light">
                  ...
                  <tbody>
                    <?php foreach ([3, 2, 1] as $lvl): ?>
                      <?php $users = $byLevel[$lvl] ?? []; ?>
                      <?php foreach ($users as $u): ?>
                        <?php $meta = $almoxLevelLabels[$lvl]; ?>
                        <tr class="<?= $meta['row'] ?>" style="opacity:0.9">
                          <td class="align-middle"><?= e($u['nome'] ?: $u['login']) ?></td>
                          <td class="align-middle text-monospace small"><?= e($u['login']) ?></td>
                          <td class="align-middle"><?= $originBadge($u['origem']) ?></td>
